...
def vae(observed, n, n_x, n_z, n_samples, is_training):
    with zs.BayesianNet(observed=observed) as model:
        ...
        z = zs.Normal('z', z_mean, std=z_std,
                      n_samples=n_samples,
                      group_event_ndims=1)
        ...
        x_mean = layers.fully_connected(lx_z, n_x, activation_fn=None)  # compute the mean
        x_logstd = layers.fully_connected(lx_z, n_x, activation_fn=None)  # compute the log std

        x = zs.Normal('x', x_mean, logstd=x_logstd, n_samples=n_samples,
                      group_event_ndims=1)
...
def q_net(observed, x, n_z, n_samples, is_training):
    with zs.BayesianNet(observed=observed) as variational:
        ...
        z_mean = layers.fully_connected(lz_x, n_z, activation_fn=None)  # compute the mean
        z_logstd = layers.fully_connected(lz_x, n_z, activation_fn=None)  # compute the log std
        ...
        z = zs.Normal('z', z_mean, logstd=z_logstd, n_samples=n_samples,
                      group_event_ndims=1)
...
if __name__ == "__main__":
    ...
    # Generate Images
    n_gen = 100
    _, x_logits = vae({}, n_gen, n_x, n_z, lb_samples, False)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for epoch in range(begin_epoch, epochs + 1):
            ...
            name = os.path.join(result_path,
                                "vae_gaussian.epoch.{}.png".format(epoch))
