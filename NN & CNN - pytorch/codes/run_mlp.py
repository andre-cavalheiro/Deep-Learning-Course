...
for conf in pipelines:
    ...
    if conf['BL']:
        model = nn.Sequential(
            nn.Linear(784, conf['width1']),
            BatchNorm1d(TRAIN_BATCH_SIZE),
            nn.ReLU(),
            nn.Linear(width1, conf['width2']),
            BatchNorm1d(TRAIN_BATCH_SIZE),
            nn.ReLU(),
            nn.Linear(conf['width2'], 10),
        ).to(device)
    else:
        model = nn.Sequential(
            nn.Linear(784, conf['width1']),
    ...
    # printResults(conf, ['BL', 'LR', 'MM', 'width1', 'width2'], acc, totalTime, 'mlp')
    printResults(conf, ['BL', 'LR'], acc, totalTime, 'mlp')
    ...
    ax.grid()
    fig.savefig('lossPlot_mlp' + 'BL='+str(conf['BL'])+' LR='+ str(conf['LR'])+' MM='+str(conf['MM'])+' .jpg')
